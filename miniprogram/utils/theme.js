...
function load() {
  try {
    return wx.getStorageSync(THEME_KEY) || 'auto'
  } catch (e) {
    return 'auto'
  }
}
...
function apply(theme) {
  ...
  // 设置 page 的 dark class
  const pages = getCurrentPages()
  if (pages.length > 0) {
    const page = pages[pages.length - 1]
    if (actualTheme === 'dark') {
      page.setData({ _isDark: true })
    } else {
      page.setData({ _isDark: false })
    }
  }
  ...
  try {
    wx.setNavigationBarColor({
      ...
      backgroundColor: actualTheme === 'dark' ? '#1d1d1f' : '#ffffff',
      animation: { duration: 200, timingFunc: 'easeIn' }
    })
  } catch (e) {
    // 某些页面可能没有 navigationBar
  }
}
